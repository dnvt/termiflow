...
Current status: TermiFlow v0.2.2 is the current public release. Print mode
...
The v0.2.2 Linux release binaries are architecture-specific: the x86_64 GNU
...
```bash
# From a checked-out v0.2.2 source tree:
cargo install --locked --path . --bin tw
```

This installs the checkout you are using. For a no-Rust installation, prefer
the v0.2.2 Homebrew formula or the prebuilt binaries linked from the GitHub
release.
