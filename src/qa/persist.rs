...
use super::persist_ops::{classify_atomic_directory_error, PersistenceOps, SystemOps};
    ...
    match (result, guard_cleanup) {
        ...
        (Err(error), Err(cleanup_error)) => {
            if final_claimed {
                Err(PersistenceError::RecoveryRequired {
                    path: target.to_path_buf(),
                    detail: format!("{error}; publication guard cleanup failed: {cleanup_error}"),
                }
                .into())
            } else {
...
pub(crate) fn replace_with_intent_with_ops<O: PersistenceOps>(
    ...
    if current_sha256.as_deref() != expected_old_sha256 {
        return Err(PersistenceError::Conflict {
            path: path.to_path_buf(),
            detail: format!(
                "old digest changed: expected {expected_old_sha256:?}, found {current_sha256:?}"
            ),
        }
        .into());
    }
